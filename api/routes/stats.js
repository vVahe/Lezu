    ...
    async (req, res, next) => {
        ...
        try {
            const q1 = await sequelize.query(
                'SELECT COUNT(*) as nr_words FROM words',
                {
            ...
            const q5 = await sequelize.query(
                'SELECT DISTINCT COUNT(DISTINCT language_id) as nr_languages FROM words',
                {
                    type: sequelize.QueryTypes.SELECT
                }
            );
            ...
            result.recent_words = await sequelize.query(
                'SELECT * FROM words ORDER BY times_incorrect DESC LIMIT 10',
                {
                    type: sequelize.QueryTypes.SELECT
                }
            );
